...
def environment(c):
    ...
    inputs = np.array([0.18*a,b,2*c,d])
...
def array_input(x):
    ...
    while i < 1000:
        result_list.append(float(environment((x[i]-2)/2)))
        i = i + 1
    return result_list
def environment_array(x):

    return array_input(x) if type(x)== np.ndarray else float(environment((x-2)/2))
