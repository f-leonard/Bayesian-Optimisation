...
class DiscreteBayesianOptimization(BayesianOptimization):
    ...
    def __init__(self, f, prange, random_state=None, verbose=2, constraints=[]):
        ...
        # Data structure containing the function to be optimized, the bounds of
        # its domain, and a record of the evaluations we have done so far
        self._pbounds = {item[0]: (item[1][:2]) for item in sorted(prange.items(), key=lambda b: b[0])}
        self._pbounds = {item[0]: (item[1][:2]) for item in sorted(prange.items(), key=lambda c: c[0])}
